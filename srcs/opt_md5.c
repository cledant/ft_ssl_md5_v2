...
static void
clean_exit(t_opt_md5 *opt, char const *str)
{
    if (str) {
        puts(str);
    }
...
parse_file(t_opt_md5 *opt, char const *str)
{
    ...
    if (!input) {
        clean_exit(opt, "ft_ssl: failed to alloc memory");
    }
...
parse_opt_args_md5(int32_t argc, char const **argv, t_opt *opt)
{
    if (!(opt->md5.queue = t_queue_new())) {
        clean_exit(&opt->md5, "ft_ssl: failed to alloc memory");
    }
